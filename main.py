...
def masg(call):
	...
	if call.data =="N_B_1":
		...
		O = types.InlineKeyboardButton(text ="(K_K_K)", callback_data="F6")
		...
		while True:
			...
			if response.text.find('If you have <strong>Telegram</strong>, you can contact <a class="tgme_username_link"')>=0:
				ok+=1
				sk+=1
				bot.send_message(call.message.chat.id,f"‹ A - يوزرات تلي متاحه ✓\n────── • ✧✧ • ──────\n‹ B - صدتلك يوزر : @{li}\n────── • ✧✧ • ──────\n• C - مطور البوت @N_B_1")
...
if __name__ == "__main__":
    bot.remove_webhook()
    bot.set_webhook(url="https://abudtsh.herokuapp.com/"+str(BOT_TOKEN))
